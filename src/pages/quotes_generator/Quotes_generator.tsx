...
const Quotes_generator: React.FC = () => {
  ...
  const [Quotes_generator, readQuotes_generator] = useState<{ id: string; title: string; description: string;dateAdded: string; }[]>([]);
  ...
  const generateRandomIndex = () => {
    return Math.floor(Math.random() * Quotes_generator.length);
  };

  // Function to generate a random message
  const renderRandomMessage = () => {
    if (randomIndex !== null) {
      return Quotes_generator[randomIndex].title;
    } else {
...
const handleAlertDismiss = () => {
  ...
  const addQuote = async () => {
    if (newTitle.trim() !== '') {
      if (editIndex !== null) {
        ...
        await addDoc(collection(db, 'Quotes_generator'), {
          title: newTitle,
  ...
  //Read Firebase Data
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'Quotes_generator'), (snapshot) => {
      readQuotes_generator(snapshot.docs.map(doc => ({
...
const editQuote = (index: number) => {
  setEditIndex(index);
  const editedQuote = Quotes_generator[index];
...
const updateQuote = async () => {
  if (editIndex !== null) {
    ...
    const quoteToUpdate = Quotes_generator[editIndex];
    await updateDoc(doc(db, 'Quotes_generator', quoteToUpdate.id), {
      title: newTitle,
      description: newDescription,
    });
...
const deleteQuote = async (index: number) => {
  ...
  const quoteToDelete = Quotes_generator[index];
  // Delete quote from Firestore
  await deleteDoc(doc(db, 'Quotes_generator', quoteToDelete.id));
};
    ...
    <IonPage className="home-page">
      ...
      {/* <img className="To_do_list-img-header" alt="Silhouette of mountains" src="./src/assets/img/quotes generator.png" /> */}
        ...
        <IonList id="list_body">
          {Quotes_generator
            .slice() // Create a shallow copy of the Quotes_generator array to avoid mutating the original array
